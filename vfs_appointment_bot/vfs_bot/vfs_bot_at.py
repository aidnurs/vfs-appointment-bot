...
class VfsBotAt(VfsBot):
    """Concrete implementation of VfsBot for Austria (AT).

    This class inherits from the base `VfsBot` class and implements
    country-specific logic for interacting with the VFS website for Austria.
    It overrides the following methods to handle Austrian website specifics:
    ...
    def __init__(self, source_country_code: str):
        """
        Initializes a VfsBotAt instance for Austria.

        This constructor sets the source country code and the destination country
        code "at"(Austria). It also defines appointment parameter keys specific
        ...
        self.destination_country_code = "AT"
    ...
    def login(self, page: Page, email_id: str, password: str) -> None:
        """
        Performs login steps specific to the Austrian VFS website.
    ...
    def pre_login_steps(self, page: Page) -> None:
        """
        Performs pre-login steps specific to the Austrian VFS website.
    ...
    def check_for_appontment(
        ...
        """
        Checks for appointments on the Austrian VFS website based on provided parameters.
